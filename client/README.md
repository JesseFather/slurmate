...
**两条投递方式**：优先 `plugin_package`（一条 RPC 取回整个 `.splug`，能自己解析、
自己重算摘要、自己验签），`files` 逐份取是退路 —— 它只在"这个包用的是本客户端读不懂
的格式"（`package.format` 比认识的**新**）时才会走。**其余每一条读包的失败都不回退**：
...
6. **签名者必须是同一把钥匙**（§5.4）—— 第一次同意一个签过名的构件时钉住它的
   公钥，此后每一份都必须相符。**逐份那条路也判**：它发的是散装字节、证不了签名者，
   所以钉过的 id 在那里一律拒（不判的话"让整包失败"就是一个绕开钉子的开关）。
...
（清单 → 逐文件 `plugin_file` → 暂存校验 → 同意闸 → 换入）本机都只用**假后端**
验过（`test/site-plugins.test.mjs`），真机上那个 4 MiB 的响应上限够不够、
限流桶够不够用，没有一次真实输出 —— 而且它现在**同时**罩着"整包一次取"
（`plugin_package`，客户端还没接上）与"逐份取"两条路。见
[`docs/KNOWN-ISSUES.md`](../docs/KNOWN-ISSUES.md) 的 **U7**。
