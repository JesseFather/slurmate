...
装与卸在 `src/main/plugins/install.js`（**本机池**那一半）。站点分发**不走它** ——
它的语义是"用户挑的目录装进用户自己的池"，与分发（站点说了算、按引用计数回收、
换入前要过同意闸）相反。分发在 [`src/main/site-plugins.js`](src/main/site-plugins.js)，
落另一个根。照旧注释去复用 `installFrom` 的人，会把"拒绝覆盖内容不同的同版本"
当成一个 bug 去修。
...
连上站点 ──► 对账（site-plugins.js）
               │  1. 问清单（plugins）—— 没有 files 字段 ⇒ 站点太旧，到此为止
               │  2. 问 list，把活会话用着的版本收进保护集（这一次不回收它们）
               │  3. 取锁 → 清暂存 → 读记录（.sites.json）
               │  4. 逐插件：已有一份且对得上 ⇒ 跳过；对不上 ⇒ 明确失败，绝不覆盖
               │             没有 ⇒ 逐文件取回暂存 → 校验 →【同意闸】→ 换入
               │  5. 写记录（在回收之前 —— 唯一会真丢数据的地方）
               │  6. 回收：没有任何站点要、也没有活会话用 ⇒ 删
               └─ 7. reload() 之后**逐个复查 registry.get(id, 版本)**
```

四条纪律，每条都对应一种"系统说成了、事情没成"：

1. **绝不谎报成功** —— 任何一份文件没到手或对不上，这次分发就报失败；报成功时，
   磁盘上的内容按字节就是站点那一份。
2. **判据是能力缺席，不是这次失败** —— "站点不支持分发"只看 `plugins` 里有没有
   `files`。拿"下载失败"当判据，等于给一个能让下载失败的人（断流、丢包、中间人）
   一个把用户降级到旧本地副本的开关。
...
★ 这四条各自都有能真的红的用例，在 `test/site-plugins.test.mjs`。
