...
把集群上的**作业**包装成桌面应用。作业里跑什么由**插件**决定 —— 现在是两个：
`code-server`（浏览器里的 IDE）与 `sshd`（SSH 中转站，给原生 VS Code Remote-SSH、
codex 这类要求 ssh 连接的工具用）。站点在 `slurmate.conf` 里用 `[plugin:名字]`
块决定装不装、开不开、默认给多少资源；客户端只画**站点开了、而自己也认得**的那些。

界面上的「开始会话」那一块是**按插件画出来的**（`src/main/plugins/` 扫出几个就画
几个），一个插件名都没写死 —— 所以站点卸掉一个插件，那一块就不见了，而不是"点了
报错"。
...
`~/.slurmate/plugins/` 是**站点分发插件的池**：任意站点的安装都丢进去，任意站点的
使用都从里面取。本版只做了结构（客户端已经会扫这个目录），**装东西的流程是下一
阶段**。
