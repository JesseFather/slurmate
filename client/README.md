...
首次使用时它生成一把**专用** ed25519 密钥，私钥由客户端自己保管：**永远加密保存**
（系统凭据库），没有第二个选项。机器上没有凭据库时不静默写明文，也不问你要不要
明文保存 —— 而是如实告诉你这把密钥存不下来、下次启动会换一把新的。公钥在界面上
常驻可查、一键复制，你把那一行加到自己的账户里就完成了唯一一次手工步骤。
