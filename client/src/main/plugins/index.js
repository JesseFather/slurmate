 ...
 *   ★ 这里从前还列着第二个读者：安装器（`plugins/install.js`）。那个文件连同
 *     本机安装那条路一起删掉了，而**它当时正是"两份抄本"的那个先例** ——
 *     它从这里引，就是为了不出现第二份。
 *
 ...
 *   那个端口（`index.js` 的 `layoutPortOf`：先查配置、再查临时组），没有的用中转
 *   基准端口。这两个值基座自己就能算，而从前那个钩子只是让插件把基座的值原样报
 *   回来。
