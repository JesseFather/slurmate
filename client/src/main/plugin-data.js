 ...
 *
 * 于是每个插件都要回答两个问题：**跨版本共不共享**（`contributes.data`）、
 * **能不能同时开两份**（`contributes.concurrent`）。两个都由**作者在清单里声明** ——
 * 只有作者知道自己的新版本读不读得懂旧数据，也只有他知道自己的代码能不能同时
 * 处理两份。
 *
 * ── 两条轴、一个有缺省、一个没有 ────────────────────────────────────────────
 *
 ...
 *   · **实例**：`contributes.concurrent: true` 才存在这一段（能同时开两份 ⇒ 每份
 *     一个实例键）；`false` ⇒ **整段不存在**，那个插件同时只有一份。
 *     存在时它的取值必须**两份不同**：常量会让两份实例落在同一个目录里各写各的
 *     ⇒ **静默损坏，而两边都以为自己成功了**。实例键今天由布局组担任（一个组 =
 *     一个端口 = 一份 origin），这就是 `concurrent: true` 要求 `layout: true` 的
 *     全部理由。
 *
 * ★ **共享组那一格落在安全侧，实例那一格没有缺省。** 这不是不一致，是两格的
 *   **基座答得了与答不了**的差别：
 *
 *   · 不声明继承 ⇒ 新版本拿不到旧数据。反过来（默认继承）会让一个改过数据格式的
 *     新版本读到自己读不懂的东西 —— 用户看到的是界面一片错乱，而不是一句"升级
 *     之后布局要重配"。**不共享永远安全**，所以缺省可以填。
 *   · **"能不能同时开两份"基座答不了** —— 它答的是"你的代码能不能同时处理两份"，
 *     而「不要 origin」（`layout: false`）与「不能同时开两份」是两件不同的事。
 *     缺省无论取哪一边都是基座替作者表态（取"不能"，一个真能多开的作者永远不知道
 *     为什么只能开一份；取"能"，一个没想过的作者会得到一个静默的第二份）⇒
 *     所以这一格**必填**，报错在 `plugins/index.js`。
 *
 ...
 *
 *   · 浏览器的存储分区 = `partitionOf(identity)` —— 段之间用 `@`，带 `persist:` 前缀
 *   · 分区目录的目录名 = `diskNameOf(identity)` —— 同一份身份，**折叠过**
 *   · 插件数据目录的目录名 = `dataDirNameOf(identity)` —— 同上，但**由我们拼**
 *
 * ★ 三个落点从**同一个身份**长出来，这是这个模块存在的全部理由：分别拼的话，
 *   "同一份数据"在分区名与目录名里会变成几个可以各自漂开的字符串。
 *
 * ★ 后两个**今天是同一个字符串**，而它们是两个函数：契约相反（见 `dataDirNameOf`
 *   的注释）。★ 它们**必须**同名 —— 对账把两个根放在一起做差，靠的就是"同一个身份
 *   在两个根下叫同一个名字"。
 *
 * ── ★ 运行期的那一半：`slotOf` ──────────────────────────────────────────────
 *
 * 上面三条回答的是"**一份数据**在哪儿"。同一个模块里还有一条**运行期**的规则，
 * 它回答的是"**一个活跃会话**占着什么"：**同一个槽同一时刻只能有一个活跃会话**
 * （`slotOf` 的注释里有逐条理由）。两者是同一件事的两面 —— 上面那条"实例段是常量时
 * 两份实例必然落在同一个目录里，各写各的"说的是**数据**，`slotOf` 说的是**进程**。
 *
 * ── ★ 磁盘上的目录名与分区名的差别：折叠 ────────────────────────────────────
 *
 ...
 *   件事（这也正是它当时不该在这里的原因）。它现在在这里，是因为三条**同时**落了地：
 *   `ctx.dataDir()`、sshd 搬进它、以及**对账长出第二个根**。任何一条单独做，它都会
 *   退回去变成一个没有读者的接口。
 *
 * ★ 顺带记下对账那侧随之而来的那条口径：`plugin-data-audit.js` 里"该有哪些身份"的
 *   入口条件从 `hasSurface`（**有没有界面**）**放宽成了"注册表里的所有插件"** ——
 *   因为数据目录这第二个根不由框架建（`ensureSurface` 那种"不看声明就建"的推论在
 *   这里不成立），而是插件自己调 `ctx.dataDir()` 建出来的，数不出来。照 `hasSurface`
 *   去算，**没有界面的 sshd 那份正在被用的数据一诞生就是孤儿**。放宽是安全方向
 *   （该有的变大 ⇒ 孤儿变少 ⇒ 删除按钮变少），而在分区那一侧**不改变任何行为**：
 *   多算出来的身份在磁盘上没有目录，不产生行。
 *
 * ── 词要钉住 ────────────────────────────────────────────────────────────────
 *
 ...
 * 这个插件的身份里**有没有实例段**。
 *
 * ★ 判据只有一处：`contributes.concurrent`（作者声明的"能不能同时开两份"）。
 *   **别在别处内联写这个比较** —— 内联一处，那一处就会在判据变动时漂开，而症状是
 *   "回收时认不出实例段" ⇒ **静默漏删**。
 *
 * ★ 它是给**清理**那条路用的：一个布局组被回收时，只有把布局组当实例段的那些插件
 *   才有"属于这一个组"的存储要清。不能多开的插件只有一份存储，它不属于任何一个组，
 *   **绝不能跟着某个组一起被清掉**。
 */
function hasInstance(plugin) {
  const c = plugin && plugin.contributes;
  return !!(c && c.concurrent === true);
}

/**
 * 一份数据的**身份**。
 *
 * @param {object} plugin  注册表里的插件对象（要 `id` / `version` / `contributes`）
 * @param {string} [instanceId] 实例段。声明了 `concurrent: true` 的插件**必须**给出来
 * @returns {string[]} `[id, 组]`，能多开时是 `[id, 组, 实例]`
 *
 * ★ 能多开却拿不到实例时**抛**，不回落成两段：那会让所有实例静默地共用一份数据，
 *   而两边都以为自己写了 —— 正是这条轴要防的那件事，而它一旦发生是看不出来的
 *   （没有报错、没有异常，只有一份坏掉的数据）。
 *
 * ★ 这个抛**够得着**，而且今天就有两条路走到它旁边：
 *   · `ctx.dataDir()` 读的是 `rec.controller.layoutId`，而 `plugin.prepare()` 从前
 *     跑在 controller **构造之前** ⇒ 一个「能多开 + 带 prepare」的插件提交就失败
 *     （见 index.js 里 startSession 的次序）；
 *   · `inspectDir` 已经拒了「能多开却没有 `layout`」的清单，`loadLayouts` 又保证
 *     每条连接都落在一个存在的组里 —— 所以**走参数化调用的那几处**
 *     （`ensureSurface` / 对账）确实拿得到。
 *   留着它，是为了让"拿不到实例"有一个**说得出原因**的下场，而不是变成一句
...
function identityOf(plugin, instanceId) {
  ...
  if (typeof instanceId !== 'string' || !instanceId) {
    throw new Error(
      `${plugin.name || plugin.id} 声明了 contributes.concurrent: true，但这次调用没有`
      + '给出实例。实例段缺失时所有实例会共用一份数据，而两边都以为自己写进去了 —— '
      + '那是一种看不出来的损坏，所以这里宁可停下来。'
      + '（实例键来自布局组：这就是 concurrent 要求 contributes.layout 的原因。）');
  }
...
/**
 * 这次会话占的那个**槽** —— 它拿走了哪一份「一个就够」的资源。
 *
 * ★ 这是运行期的不变量：**同一个槽同一时刻只能有一个活跃会话。**
 *   它是 `identityOf` 那条"实例段是常量时两份实例必然落在同一个目录里，各写各的"
 *   在**运行期**的对应物 —— 那一句写在清单校验里（`concurrent` 要求 `layout`），
 *   这一句写在活跃会话之间。
 *
 * ── 为什么只有两种槽 ────────────────────────────────────────────────────────
 *
 * `layout:<组 id>` —— 要布局组的会话。一个组 = 一个本地端口 = 一份 origin = 一份浏览器
 *   存储，所以**同组的第二个会话无论是不是同一个插件都不行**：端口是同一个、分区也是
 *   同一个。组今天只有一个来源（活跃连接的那个），所以两条这类会话必然同槽。
 *
 * `relay` —— **全部**不要布局组的会话，整类共用一个槽。★ 这一档**故意**粗。
 *   因为在这一类里，「互不干扰」不是基座保证的，而是**插件自己的口头承诺**：
 *   写进用户 ssh 配置的那个别名（`plugins/sshd/client/sshconfig.js` 的
 *   `SSH_ALIAS`）是插件自己的常量 —— 基座**无从核对**两个无组插件的别名会不会撞。
 *   核对不了的事只能整类互斥：粗一档拒绝并说出原因，细一档（按插件 id 分槽）会让
 *   两个互不认识的无组插件并存着去抢同一个别名，而症状是"`ssh slurmate` 连到
 *   哪一个是不确定的"，两边都报"已就绪"。
 *
 *   ★ **这一档从前还有第二个理由（端口），它已经不成立了**：那个理由是"端口由插件
 *     的 `preferredPort` 自己挑"，而那个钩子后来收掉了 —— 现在无组会话的端口是基座
 *     给的常量（`RELAY_PORT_BASE`），基座**核对得了**。剩下的只有别名这一半。
 *     ⇒ **别**因此就把这一档按插件 id 拆细：拆细的前提是插件把别名也交给基座声明
 ...
 * @returns {string} 槽的名字
 */
function slotOf(layoutId) {
  return layoutId ? `layout:${layoutId}` : 'relay';
}

/**
 * 身份的**浏览器落点** —— Electron 的存储分区名。
 *
 * 结构是"每个插件一份，同一份身份共用"：`concurrent: true` 的插件，实例不同的
 * 两次会话拿到两个分区（于是能同时开）；不能多开的只有一份（于是两份会话写的是同一个
 * 目录 —— 那一格本来就不该同时开）。
 *
...
/**
 * 身份的**插件数据落点** —— 基座给它建的那个目录名（`ctx.dataDir()` 就是它加上根）。
 *
 * ★ 三段用 `@` 拼成**一个名字**，**不是** `path.join(...identity)` 那样的三层目录。
 *   三层会让 `<id>/<组>` **既是两段身份的数据目录、又是三段身份的父目录** —— 而两份
 *   身份**真的会同时出现**：同一个插件 1.0.0 声明 `{inherit:'editor'}`（两段），
 *   2.0.0 声明 `{inherit:'editor', concurrent:true}`（三段），而 `inherit` 的语义
 *   恰恰是"这几个版本共享一份"。于是"删掉没人用的 editor 那一份"会把 2.0.0 那份
 *   **正在用的**连根 `rm -rf` 掉，而两边都不报错。单段名字让两份身份永远是**兄弟**，
 *   而这条不变量是"名字是单段"的直接推论，不需要任何检查去维持它。
 *
 * ★ 它今天是 `diskNameOf` 的**同值函数**（两个根下的同一个身份必须同名，对账的两个
 *   根共用一张"该有的"表靠的就是它），而**两个函数都要留**：契约相反 —— `diskNameOf`
 *   写着"**绝不**用来拼路径"（路径一律由 `readdir` 拿到的原始名拼），这一条**只用来
 *   拼路径**。合并之后"用谁拼路径"会在每个调用点重新回答一遍。
 *
 * ★ 于是这里**刻意重写一遍那个表达式、而不是转调 `diskNameOf`**：那样写的话"两者同值"
 *   会恒真，也就没有任何东西守着它；分开写，它是**可违约**的，而 `test/plugin-data.test.mjs`
 *   里那条等式会在有人只改一边时变红。
 */
function dataDirNameOf(identity) {
  return foldAscii(identity.join('@'));
}
