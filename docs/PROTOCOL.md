...
**当前版本：v0.6。** 协议版本、客户端版本、服务端版本是**同一个号** —— 它们不是
...
超上界时**必须进位**（`0.255` 之后是 `1.0`）。所以它是 `v0.6`，不是 `v0.6.0`。

> ★ **两段是有意的，而且这是这个项目里两套版本号中的一套。** 另一套是**插件**
> 的版本号（`x.y.z`，见 [PLUGIN-SPEC.md](./PLUGIN-SPEC.md) §2.3），它比这个号
> 多一段。两套的形状不同、用途不同，**只有一条共同的纪律**：逐段按十进制字符串
> 比较、禁止转机器整数。写得下"补丁级"的只有插件那一套；框架这一套没有第三段。
>
...
> 的，那句已经改掉了：语义化版本要求三段，而这里只有两段。这不算什么损失 ——
> 这个号本来就只回答"两端是不是一起升的"，不承诺 API 兼容性；协议兼容性由下面
> 这张变更表负责。）

> ✅ **两端都已按 v0.6 实现**，四处版本号都是 0.6。此前这里挂着一条警告说集群侧
...
1 + 1 + N 次 RPC 降到 1 + 1 + 1 次。**今天的客户端已经走整包那条路了**，而
**删掉 `files` 与 `plugin_file` 排在后面那一段** —— 那才是一次真正的不兼容变更。
在那之前，两条路都必须是真的能用的：客户端**只在**"这个包用的是它读不懂的格式"
（`package.format` 比它认识的新）时才退回逐份取，而**其余每一条读包的失败都不回退**
—— 一个验不过的包加上一条能绕过它的路，等于把验签降级成一句建议。
...
成功：`{"pong": true, "version": "0.6", "time": 1757...}`
...
- **★ 现在，这份清单来自站点的那个插件包**（`<ULID>.splug` 里那张记录表），
  不是"遍历一遍插件目录"算出来的。字段与语义**一个字都没变** —— 老客户端照旧
  按 `plugin_file` 逐份取，所以这是一次**加法过渡**（见〈七、兼容性〉与上面的
  版本表）。
...
- **`path` 只是一个键。** 守护进程那一侧它是索引表的键（`plugin_file_index()`），
  ...
  任何一条不过就**拒绝整份**。
- **符号链接与空目录不进清单。** ★ 插件变成包之后这不是一条纪律，是**结构性的**：
  包里的负载是一条条 `路径 | 字节`（`docs/PLUGIN-SPEC.md` 附录 A.3），链接与目录
...
> 一次对账从 1 + 1 + N 降到 1 + 1 + 1。**今天的客户端走的就是它**，所以正常路径上
> 两个插件是 3 次而不是 11 次。
>
> ★ **但上面那条"串行 + 退避"的纪律仍然必须遵守** —— 它仍然是一条**在用的**路：
> 客户端在"这个包用的是它读不懂的格式"时会退回逐份取（格式演进的加法过渡），
> 而老客户端一直走这条。留着它的那个理由（限流桶按"人点一下按钮"设计）一个字
> 都没变。
...
| `service_kind` | 这个会话提供哪种服务 —— **本站的短名**（配置块名）。三态：字符串 / `null`（服务端**明说**不知道，会话是从 nft 规则恢复出来的）/ 键不存在（老守护进程）。 |
...
> 得掉）。这和"字段不存在"（老守护进程）是两回事 —— 后者落到客户端那条
> `legacyDefault` 兜底上：老守护进程连服务种类都不报，那时它只可能产生那个唯一
> 可用服务的会话，而清单里标了 `legacyDefault: true` 的插件就是它。**标了才有，
> 没装就没有** —— 这个贡献位不随包发行，见 [plugins/README.md](../plugins/README.md)。
