...
客户端 ──ssh -T──► 登录节点上的 slurmate stream ──AF_UNIX──► slurmate-sessiond
                    （stdin 一行一条，stdout 一行一条） （同一条连接，双向）
```
...
ssh -T -o BatchMode=yes -p 10100 alice@node01.example.com \
    -- "/bin/bash -c '/usr/local/bin/slurmate stream'"
```

- stdin **一行一条请求**，可以带 `rid`；stdout **一行一条**：响应带 `rid`，
  通知带 `seq`（判据见〈二·五〉）。
- 内部只维持**一条**到守护进程的连接，不再每条请求一个 SSH exec + 一个
  Python 解释器（实测：一次 `slurmate` 进程启动中位 **57.5 ms**，而其中
  19.3 ms 是解释器本身的地板）。
- **`stream` 是一个字节泵**：不解析、不校验、不重排、**不做任何用户态缓冲**
  （读多少写多少）。这是整条背压链上唯一可能引入无界缓冲的地方，所以它必须
  没有 —— 链条是：客户端不读 ⇒ 它阻塞在 write(1) ⇒ 它不再 read(socket) ⇒
  守护进程拿到 `EAGAIN` ⇒ 按水位丢**推送**（响应绝不丢）。中间任何一级加了
  队列，这条链就断了，而症状是"内存慢慢涨"，指不回任何一行。
- 它**不加限速**、**不加空闲超时**。前者是因为它以用户自己的身份跑，限自己
  防不了任何东西，而"一个判据两处实现会漂"，漂的方向是心跳发不出去 ⇒ 被判定
  孤儿 ⇒ `scancel`；后者是因为它**不理解心跳的语义**（谁在心跳、多久算超时，
  是客户端 `session.js` 的事），按"多久没说话"在这里关连接，会在用户只是停下来
  看界面的时候把通道掐掉。
- 两个方向的限长**不一样**，而且都不由 `stream` 自己实现：请求 64 KiB
  （守护进程的 `CONN_IN_MAX`），响应 4 MiB（`RPC_MAX_RESPONSE_BYTES`）。
- 连上之后**不再有任何超时**（`STREAM_CONNECT_TIMEOUT = 40` 秒只管连接那一步）。
- 半关（stdin EOF）**不等于断开**：已经发出去的请求还要把响应收回来。若之后
  守护进程把连接关了 ⇒ 正常终点，退出码 `0`；没半关过就被关 ⇒ `5`
  （守护进程不在了，客户端该重连）。
...
只在 `slurmate stream` 上存在。`rpc` 那条路上一个字节都没有变。

#### `rid` —— 请求与响应的配对（加法）

请求可以带一个 `rid`（字符串或整数）。带了的，响应把**同一个值**原样回填：

```json
{"op": "status", "rid": 7}          →  {"ok": true, "code": 0, "data": {...}, "error": null, "rid": 7}
```

**不带就不回填。** 所以老客户端的请求与响应在线上与从前逐字相同。
...
判据是 `rid` 而不是"连接活了多久"或一个 `subscribe` op，理由是：老客户端
（`slurmate rpc`）每次都是**新连接、只发一条、永远不带 `rid`**。把订阅挂在这上面，
"老客户端收不到推送"就成了一个**结构事实**，而不是一句承诺 —— 否则一次恰好落在
对账时刻的通知会被它当成响应读走，症状是随机的、说不清的失败。
...
```json
{"op": "list", "rid": 3, "client": {"id": "m9f3c1…", "name": "笔记本"}}
```

- `id` 是**稳定的**（落在客户端数据目录里的一个随机串）。**判等只用它。**
- `name` 只用于显示，缺省是主机名。一台机器改个名不该被当成换了一台电脑 ——
  那会让它自己那份席位失效，而症状是**它被自己顶掉**。

#### ★★ `client` 与 `rid` **在同一个位置、同一条理由上被认领**

| 这条连接 | 算不算一个「客户端」 |
|---|---|
| 发过至少一条**带 `rid`** 的请求 | **算**（前提是带了 `client`） |
| 一条都没发过 | **不算** —— 即使带了 `client` |

理由与上面那条订阅的判据**逐字相同**：`slurmate rpc` 是客户端在常驻通道起不来时的
**退路**，而它永远不带 `rid`。它若也能认领身份，那么客户端每退路一次就以同一个 `id`
报一次名，服务端会把**正在用的那条常驻通道**当成「同一个客户端又连上来了」顶掉 ——
于是每降级一次 exec，就把自己那条常驻通道打掉一次。症状是「通道时好时坏」，
而两边各自的日志都自洽。

⇒ **认不出身份的连接既不占席位、也永远不会被顶掉。** `slurmate rpc`、`slurmate wait`、
VS Code 的 Remote-SSH、你的终端 —— 同一个用户在同一个站点上本来就会有很多条 SSH
连接，它们一条都不该被顶掉。「两条 SSH 连接」和「两个客户端」是两回事。

#### 席位的规则

- 每个 uid 有 `max_clients_per_user` 个席位（见 [CONFIGURATION.md](CONFIGURATION.md)）。
- **后到者赢**：超编时按 **`client.id` 首次出现**的时刻从旧到新顶掉 ——
  **不是**按连接建立的先后，**不是**按活动度。
  ★ 按活动度会让两台电脑在都在用的时候互相顶（你在这台上动一下鼠标那台就断）。
  ★ 按连接先后的话，一次**重连**（笔记本休眠醒来、网络闪断）会把没出任何问题的
  那台顶掉，而对方连自己为什么掉了都不知道。
- 同一个 `client.id` 的第二条连接**让位给新的那条**（异常兜底：上一次的连接还挂在
  服务端这边）。这是同一条席位内的替换，不占额外名额。
- 被顶掉的 `client.id` **同时从席位表里删掉**，于是它下次连进来**算新人** ——
  ★ 这正是「被顶掉的那个禁止自动重连」必须成立的理由（见下）：少了那一条，
  两个客户端会互相顶，**没有终点**。

#### 顶替通知的形状

```json
{"push": "displaced", "seq": 31, "at": 1758800000,
 "reason": "另一个客户端接管了",
 "by": {"id": "m1a2b3…", "name": "台式机"}}
```

- 它**没有 `sessions`**，只有 `reason` 与 `by`。`by.name` 是**必须**的 ——
  没有它，用户只看到一次没有原因的断线，下一步是重启客户端，然后再被顶一次。
- `seq` 与快照通知**共用**同一个计数（每条连接各自单调）。
- ★★ 它**不看水位、也不走 `enqueue_push`**：它是一次性的、不可重放的。被丢掉之后
  客户端只会看到一次**没有原因的**断线 —— 然后自动重连，而那正是拉锯的燃料。
  它很小，而水位之上压着的是**可以被丢掉**的全量快照。

#### ★★★ 顶替只断连接，**一个字都不动会话**

服务端做的全部事情是：发一条 `displaced`、停读、排空、关掉那条连接。

- **不发** `goodbye`、**不**改会话状态、**不**删 ACL、**不** scancel。
- 会话与作业原样留在集群上；新客户端连上来之后靠 `list` + `reattach` **接手心跳**
  （窗口由 `suspect_after` 覆盖）。**"换一台电脑看"不该让作业断掉**，
  这是这一节唯一要保证的事。
- ★ 而它有另一半在**客户端**：被顶掉的客户端**绝不能**给这些会话发 `goodbye` ——
  `goodbye` 会让 `phase_release` 删掉 ACL **并 scancel 作业**。服务端分辨不了那个
  `goodbye` 是谁发的（它按 `session_id` 记账，不按连接），所以那一半只能在客户端堵。

#### ★ 被顶掉的连接，在途的那些请求也不再受理

服务端在顶替的同时停读，而且**不再处理它已经收下、还没处理完的行**。少了这一条，
一条被顶掉的连接能在临死前提交一个作业 —— 而那条连接已经不属于原来那个客户端了。

#### 通知的形状

```json
{"push": "sessions", "seq": 12, "at": 1758800000,
 "sessions": [ ... 与 `list` 的 `data.sessions` 逐字同构 ... ],
 "stale": {"dropped": 2, "from": 10}}
```

- **判据：有 `push` 键且没有 `rid` ⇒ 通知；有 `ok` 键 ⇒ 响应。** 两者不会混淆：
  响应永远带 `ok`，通知永远不带。
- `at` 是这份视图取自哪一时刻（秒）。**客户端据此看得出画面有多旧。**
- `stale` **只在丢过通知时出现**，见下。

#### ★★★ 只推全量，绝不推增量

每条通知都是一份**自足、完整**的会话列表。于是**丢掉任何一条都无害** ——
下一条会纠正它。这一条是整套背压能这么简单的原因：丢弃不需要分级、客户端不需要
重放，"重连恢复"与"落后恢复"是**同一条代码路径**。

改成增量的话，"丢弃"会从无害变成有害，而症状是**界面上静默地少一个会话** ——
不报错、不重连，只是少一行。

#### `seq` 与 `stale`

`seq` 是**每条连接各自**的单调计数，每一条通知 +1。

水位（见下）挡下一条通知时，它**照样占一个 `seq`** —— 于是**缺口本身就是凭据**：

```
收到 seq  1 2 3 5      末−首+1 = 5 = 收到 4 条 + stale.dropped 1
```

`stale: {"dropped": N, "from": M}` 搭在**下一条能发得出去**的通知上，自己不单独成为
一条消息 —— 水位高的时候它同样发不出去，那是个悖论。`from` 是缺口的第一个 `seq`。

#### ★★★ 通知**不计入**限流桶

桶（`max_rpc_per_second`，按 uid、1 秒滑窗）在 `dispatch` 里，而**通知根本不从
那里走** —— 它是一条独立的入队路径。不写成 `if op != "push"`：那是在调用点做例外，
下一个人会加第二个。让通知不走 `dispatch`，例外就变成一个结构事实。

这不是洁癖。桶对所有 op 一视同仁，而 `heartbeat` 就在里面。通知一旦计进桶：

```
通知占满桶 → 心跳被回 rate_limited → 服务端记的 last_hb_at 不更新
...
| 水位 | 值 | 到了会怎样 |
|---|---|---|
| 软 | 64 KiB | **摘掉读侧** —— 不再接受新请求。在途响应因此天然有界 |
| 硬 | 4 MiB | **丢通知**（≠ 丢响应）。丢的同时 `seq` 照常前进、`stale` 记数 |

硬线**必须等于** `RPC_MAX_RESPONSE_BYTES`（4 MiB）：合法响应最大的是
`plugin_package`（base64 之后约 2.7 MiB），硬线比它小的话，一条**合法响应**会因为
"队列里还压着一条快照"而进不去 —— 症状是插件装不上，而报错会指向守护进程没响应。
这个等式由 `cluster/test-sessiond-logic.py` 逐字比对（三处：守护进程、CLI、客户端）。

#### 一句必须写下来的语义

> **通知说的是「上一次对账时的视图」，不是实时查询。**

它最多旧一个 tick（2 秒）—— 渲染时只读本 tick 已经取过的作业信息，一次
`scontrol` 都不发。理由不只是省：按连接渲染的代价是 O(会话 × 连接)，那会把
"常驻通道"变成"常驻风暴"。
...
- **通知的 `sessions` 截断到最近 50 条**，与 `list` 同一条规则。所以它是
  「**这一批**里有什么」，**不是**「你一共有哪些会话」——
  ★ **在这里找不到某个 `session_id`，不构成"那个会话没了"的证据**，谁都不许这么读。
  ★★ 而 `list` 帮不上忙：**它同样截断到 50 条**。能回答"这个会话还在不在"的
  只有一条路 —— **带 `session_id` 的 `status`**（它按 id 取，不经过那个截断）。
- **没有变化也会推。** 周期 30 秒，且**必须严格小于客户端的 `STATUS_MS`（60 秒）**
  —— 否则这次改动在"数据新鲜度"上是净退化。客户端的看门狗再取它的两倍多
  （`PUSH_STALE_MS = 75 秒`）：**两个数的大小关系由 `cluster/test-sessiond-logic.py`
  逐字比对**。

#### 一次往返可以很慢，而协议**不设**超时

`submit` 会 fork 一次 `sbatch`，合法地跑上几十秒。所以：

- **协议层没有请求超时。** 一条响应可以在很久之后才回来，`rid` 保证它不会串到
  别的请求上（响应**允许乱序**）。
- 超时是**调用方**的事，每个调用点各自有各自的语义超时（`submit` 45 秒、
  `goodbye` 10 秒）。客户端那条常驻通道另有一个**兜底**超时，它刻意取得比任何
  调用点都长 —— 取短了它会**抢先**切断一条合法请求，而症状是"提交超时"，
  一句把根因指向集群的话。

#### 连接层

- **同时最多 64 条连接**（与 `listen()` 的 backlog 一致）。超限的那条会被
  **accept-then-reject**：它**读得到** `5 too_many_connections`。
  ★ 只"不 accept"是不行的：客户端 `connect()` 仍然成功，然后静默等到 40 秒超时，
  报一个方向完全反了的 `daemon_unreachable`。
- **半关（读到 EOF）不是断线。** 只停读，连接留着，等出站排空 —— 老客户端
  **每次发完就半关**，在那里关连接会把 `submit` 的响应一起带走 ⇒ 客户端拿不到
  `job_id` ⇒ 重试 ⇒ 被 `quota_active` 拒 ⇒ **用户看到「提交失败了」而作业在跑**。
  没订阅过的连接排空即关；订阅过的留 `EOF_LINGER`（10 秒）。
...
| `5` | **守护进程暂时给不了**：不可达、端口池空、常驻连接已满 | `daemon_unreachable`、`no_port`、`too_many_connections` | `5` |
...
### 4. 本机已被顶掉 —— `kind == "displaced"`（**客户端合成，守护进程从不发**）

它不是一次请求的失败，而是一种**状态**：另一个客户端接管了这个站点上的席位。

**处理：停止重试，并**明确**告诉用户是被谁顶的。** 与上面第 1 条（传输失败）
**必须分开**：传输失败是暂时的（我们在重连，下一轮可能就好了），被顶掉是**终局**的
—— 在用户手动点「连接」之前，重试一万次都是同一个结果。混起来的话，界面会一直显示
「重试中……」，而它永远不成功、也永远不说清为什么。

★ 它还有一条更重的后果：**被顶掉的客户端一律拒绝发请求，绝不退回 `exec`。**
退回去的话它会安静地继续干活、界面完全正常，而「你已经被另一台电脑接管了」
**一个字都不会出现** —— 一个只在协议层成立、在界面上看不见的状态。

（`client/src/main/backend-ssh.js` 的 `displacedError` 与 `classify.js`。）
...
并发的第二个提交当场被拒。修法是**完备**的，唯一的依据是守护进程**不可重入**
（`dispatch()` 在事件循环线程里同步跑到底，内部**没有任何让出点**），所以两次
`submit` 不可能交错；而那道检查在 `store.insert` **之前**
（见 [ARCHITECTURE.md](ARCHITECTURE.md)〈六之二〉最后那一节）。
...
| `job_reason` | 排队原因（Slurm 的 `Reason` 原话，如 `Resources` / `JobHeldUser`）。**`Reason=None` 被服务端滤掉**，所以它只在真有原因时出现 |
| `job_exit_code` | Slurm 的 `ExitCode` 原话（`"143:0"` / `"0:0"`）。**没结束时也有值**（恒为 `0:0`）—— 该不该显示是调用方的判断 |
| `job_restarts` | Slurm 的 `Restarts` 原话（字符串形式的整数）。**重启过就要说出来**：作业被重新排队之后跑着的东西是新的 |
| `auth_password` | 只在会话处于 `ACL_STATES` 时返回 |

> ★★ **`job_terminal` 是判定，不是状态名 —— 客户端不许自己判。**
>
> "这个作业算不算结束了"取决于 Slurm 的状态**以及** `Requeue`：`PREEMPTED` 与
> `TIMEOUT` 只在这两种附加条件下才终态。客户端手上没有 `Requeue`，所以把判定发过去，
> 而不是把原始字段发过去。自己再实现一遍的代价是**两处判据会漂**，而漂的方向是
> 界面说"已结束（被抢占）"、作业几分钟后又回来了 —— 用户已经被告知结束了。
>
> `job_state` 仍然照发（界面对不上时要说得出原文），但它**不是**判定依据。
> 判据只有一份，在守护进程的 `job_is_terminal()` 里，两处调用（`phase_running()`
> 与 `session_view()`）用的是同一个函数。
>
> ★ **这个键缺席 = 老守护进程**（0.7 及更早）。那时客户端**印状态原文**，不猜 ——
> 猜一个的代价是说错话。见 `client/src/main/jobstate.js`。
