...
**当前版本：v0.4。** 三处版本号（`client/package.json`、`cluster/slurmate`、
`cluster/slurmate-sessiond`）由 `.github/workflows/checks.yml` 断言必须一致。

> ✅ **两端都已按 v0.2 实现**，三处版本号都是 0.2.0。此前这里挂着一条警告说集群侧
> 还停在 v0.1、两端连不上 —— 那条现在不成立了，所以删掉：留着一条已经不成立的警告，
> 与留一条已经失效的注释是同一类问题。
>
> ⚠️ 但 **v0.1 的客户端仍然连不上 v0.2 的守护进程，反之亦然**。升级时两边要一起升。
...
成功：`{"pong": true, "version": "0.2.0", "time": 1757...}`
...
> 得掉）。这和"字段不存在"（老守护进程）是两回事 —— 后者按内建插件的短名兜底，
> 因为那时的守护进程只可能产生内建插件的会话。
