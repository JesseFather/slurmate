...
你在另一台电脑上打开了 Slurmate，这一台被**顶掉**了 —— 同一个站点上每人
`max_clients_per_user` 个席位（缺省 1），超出的按**后到者赢**。

**先说最要紧的一句：你的作业一条都没事。** 顶替只断开这一台那条常驻连接，
**不发 `goodbye`、不改会话状态、不删 ACL、不 scancel**（见
[PROTOCOL.md](PROTOCOL.md)〈顶替只断连接〉）。会话仍然在集群上跑着，另一台已经
接手了心跳。

在这台电脑上接着管，就点那条连接上的「连接」。★ 而**这台不会再自己连回去** ——
那是设计：自动重连会让两台电脑互相顶、**没有终点**（见 KNOWN-ISSUES 的 S28）。

| 现象 | 怎么回事 |
|---|---|
| 界面上会话还在，但状态条写着"已被另一台电脑顶掉" | 正常。作业还在跑，只是这一台不再管它了 |
| 点「停止会话」报一句"这个动作没有发出去" | 也是设计：被顶掉的客户端**一律拒绝发请求**，绝不退回 exec —— 退回去的话界面会一切正常，而"你已经被接管了"一个字都不会出现 |
| 会话详情里多了一行「本机：已下线」 | 同上。★ 注意「作业状态」那一行**仍然是 running**，那是对的 |
| 想两台同时看着 | 让管理员把 `max_clients_per_user` 调到 2 或更大（见 [CONFIGURATION.md](CONFIGURATION.md)） |

★ 守护进程那一侧能看到谁顶了谁：

```bash
journalctl -u slurmate-sessiond | grep -i '被顶掉'
# 或审计日志里那两个事件：client_joined / client_displaced
```
...
**那不是"没有"，是"我们没问到"** —— 两句不同的话，界面上也画成两种样子。
协议里每一格都是三态的（`docs/PROTOCOL.md` 的 `cluster` 一节）：
**键缺席 = 取不到**，`null` / `[]` / `{}` = **确实没有**。
...
★★ **先跑 `slurmate-sessiond --check`。** 它会打分区、节点忙闲、队列与 GRES 四段，
而那正是判断"是集群的问题还是配置的问题"唯一能一次看清的地方。
★ 这几个命令的**路径**是在**启动时**校验的（配置里留空自动查找，写死就必须存在）——
所以"取不到"通常不是路径问题，而是**控制器/账本库真的答不上来**。

**为什么这几格是"旧"的**：它们来自守护进程里那份**三层钟的缓存** ——
控制器健康 2 秒刷一次，节点忙闲与队列 30 秒，分区表 / GRES / 账户 / 公平份额 5 分钟。
所以一格的数最多旧 5 分钟，而**每一格都带着"取到它的时刻"**（界面按"多久以前"显示）。
★ 这是**有意**的：一次查询服务所有连接，请求路径上一次 `scontrol` 都不发。
理由与代价见 `docs/ARCHITECTURE.md`〈六之五〉。

### ★ 「最近作业」报「取不到作业历史」

它**故意不跟着上面那张表刷新**：`sacct` 是这一组里最贵的一条查询（账本库要按时间窗
扫描），而它回答的"过去发生了什么"不会自己变新。点一次拉一次。

★ 报错而**不是**空列表：空列表的意思是"你这几天没有作业"，一句我们并不知道的话。
★ 最可能的原因：这个站点上**没有开记账**（`AccountingStorageType` 是空的），
或者你的作业比配置的窗口（缺省 7 天）更早。
