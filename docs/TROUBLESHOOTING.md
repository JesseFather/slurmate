...
第三种最容易被当成前两种。客户端把站点有、而自己没有的插件列在
`plugins.unknownToClient` 里并会在打开会话时点名 —— 那是升级提示的唯一来源。

### 守护进程起不来，报「[plugin:xxx] 是未知的插件」

块名打错了（`[plugin:ssh]` 少一个 d 也算），或者你装的是**站点分发**的插件而它
还没被认出来。报错里会列出本版认得的名字。守护进程**故意**不静默忽略 ——
静默忽略的后果是「配置里写着，而实际什么也没开」。
...
### 客户端启动时提示「插件目录里有文件被跳过」

`client/src/main/plugins/` 下的某个 `.js` 加载失败、缺字段、或者导出的 `name` 与
文件名不一致。**这是设计好的行为**：跳过它、记一条、其余插件照常工作 —— 一个坏
插件不该把客户端带崩。提示里会写清是哪个文件、坏在哪。
