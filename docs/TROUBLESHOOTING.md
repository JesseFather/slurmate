...
| 界面上说「本机有 N 份插件数据没人在用」 | 那是**分区**（不是布局组）：属于一个已经删掉的布局组、或者属于一个已经不在本机的插件版本。它们**不会再被任何东西读到**，所以可以删掉；删掉**找不回来**（那个插件下次打开是一份全新的空白存储）。想重置**正在用**的那一份，用布局下拉里的「＋ 新建空白布局…」 |
...
- **不会**：动正在被那块界面用着的那个分区。`clearLayoutStorage` 拿
  `win.surfacePartition` 对一下，是它就直接豁免。少了这一步，用户当前的 IDE 会连
  cookie 带 localStorage 一起被抽掉，而症状只是「页面莫名其妙坏了」。
  ★ 而**用户主动删掉一份数据**是另一条路（界面上那个「删掉这一份」）：那一条碰到同样
  的情形会**明确拒绝**并说清原因 —— 因为"静默跳过"在那条路上等于"点了没反应"。
