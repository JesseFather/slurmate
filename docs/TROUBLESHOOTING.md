...
| 「站点给的逐份清单与整包说的不是同一份东西」 | 站点对同一份插件给了两个互相矛盾的说法 | 让管理员重新部署那一个插件。**客户端不挑一个信** |
| 「这一份包读不了（…）」 | 包在传输或打包时坏了 | 让管理员核一下那个 `.splug`（`packer verify`） |
| 「这一份包用的是格式 N，而本客户端只认识 1」 | **客户端比站点旧** | 升级客户端。★ 这一条**会退回逐份取那条路**，所以插件照样装得上 —— 只有站点不再发文件时才变成一句失败 |
| 「这个 id 你以前同意的是 X 签的，而这一份是 Y 签的」 | 签名的人换了（§5.4） | **先别同意**。找作者核对那两串指纹 —— 正常情况下这不该发生（丢了私钥的作者只能给插件铸一个新 id） |
| 「这一份没有签名」而以前同意过一份带签名的 | 那个 id 被钉在一把钥匙上，而这一份证不了它是同一个人 | 同上。★ 逐份取那条路**也**会撞上这一条 —— 它没有被排除在外，这是刻意的 |

★ **下载失败绝不退回本机池。** 判据是"站点有没有这个能力"（协议里 `files` 字段在
不在），不是"这次成没成"。把两者合并，等于给一个能让下载失败的人（断流、丢包、
中间人）一个把用户降级到旧本地副本的开关 —— 攻击成本从"改内容"降到"让下载失败"。

★ **限流（`7 rate_limited`）不是失败。** 正常路径上一次对账是 3 个请求（1 个清单 +
1 个会话列表 + 1 个整包），已经不在桶边上了；但**退回逐份取**（老站点、或者上面
"包格式太新"那一条）时它是 1 + 1 + 每个文件一个 —— 十来个，正好压在限流桶边上。
客户端会自己退避重试，不把它记成失败。
