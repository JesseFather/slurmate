...
| 按钮不在，别的机器上有 | **本机把它关掉了**（`<userData>/config.json` 的 `plugins` 段，键是插件的 `id`） | 自己打开 |
...
> ★ 这张表里**从前还有一行**：「界面上说『本机的开发者模式关着，而池里有』」，
> 对应的是"你自己往 `~/.slurmate/plugins/` 放过一份，去把开关打开"。**那一行连同
> 它描述的那条路一起删掉了**（§5.1/§5.2）：那个目录不再被读，而开发者模式今天换的是
> **整个后端**（连不连集群），与"多加载一个插件目录"没有关系。留着那一行，
> 用户会照它去开一个开关，而开关打开之后什么都不会变。
...
★ **下载失败绝不退回池里已经有的那一份。** 判据是"站点有没有这个能力"（协议里顶层
有没有 `limits`），不是"这次成没成"。把两者合并，等于给一个能让下载失败的人（断流、
丢包、中间人）一个把用户降级到旧副本的开关 —— 攻击成本从"改内容"降到"让下载失败"。
★ 从前这句话说的是"退回**本机池**那一份"，而本机池在 v0.7 删掉了。**纪律没变，
换的只是"退回到哪一份"** —— 池里那一份仍然可能是旧的（站点升了版本而这次没取成），
所以这条判据一个字都不用改。
...
### 1. `~/.ssh/config` 里那一行 Include 指向哪儿

```bash
head -3 ~/.ssh/config
```

它必须是一条指向**插件数据目录**的 `Include`（由客户端自动维护，永远在最上面）。
如果它指向 `~/.slurmate/ssh/config`，那是 0.7 之前的位置 —— **重新开始一次会话**就会
自愈（每次建立中转站都会把那一行改成对的）。

★ 悬空的 Include **不致命**（ssh 会忽略它继续读后面的内容），所以那一行万一指错了，
症状不是"全部 ssh 都不通"，而是只有 `slurmate` 这个名字不认。

### 2. 那把一次性钥匙是哪一把

★ **0.7 升级之后，如果你那一刻正好有一个中转站会话活着，它需要重开一次。** 钥匙搬了
家（从 `~/.slurmate/ssh/id_ed25519` 搬到插件的数据目录里），客户端在新位置找不到就会
**重新生成一把**，而作业里 `authorized_keys` 装的还是**旧那一把** —— 症状正是
`Permission denied (publickey)`。重开一次会话即可：新作业装的是新的公钥。

### 3. 旧位置还剩东西吗

那三个文件（`config`、`known_hosts`、`id_ed25519`）在新位置写成功之后会被自动清掉；
删不掉时客户端会提示一次，而**删不掉不影响使用**。

★ 别去删 `~/.slurmate/ssh/host_ed25519` —— 那是**作业侧**的主机密钥（在集群账号的
家目录里，与客户端那三个文件不是同一个目录）。删了它客户端会判定为中间人攻击。
...
插件目录下的某个插件不合规，被跳过了。客户端**只有一个**插件目录：

| 目录 | 是谁的 |
|---|---|
| `~/.slurmate/site-plugins/` | **站点分发的**（可以按引用计数回收，你手改它没有意义 —— 下次对账会按站点的版本复原或报错） |

> ★ 这里从前还列着第二个：`~/.slurmate/plugins/`（"本机池"，用户自己放进去的，
> 靠开发者模式那个开关加载）。**那个目录不再被读**，那条路也删掉了（§5.1/§5.2）——
> 里面**已有的文件不删**，但下一次启动会说一句它不生效了。
