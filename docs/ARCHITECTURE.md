...
两条轴、两个声明、两个缺省都落在安全侧：`contributes.data.inherit` 缺省 ⇒ 身份里带
版本号（每个版本一份）；`contributes.data.perInstance` 缺省 ⇒ 没有实例段（所有实例
一份，于是天然不许同时开两份）。**"共享"不是三选一的模式**，而是同一个声明写了什么
（都写同一个组名 = 多对一；分组写 = 多组对多组；不写 = 每个对每个）。
完整的契约在 [PLUGIN-SPEC.md](PLUGIN-SPEC.md) §2.7。
...
| `relay` | **全部**不要布局组的插件 | ★ 这一档**故意粗**：写进用户 ssh 配置的那个别名（`plugins/sshd/client/sshconfig.js` 的 `SSH_ALIAS`）是插件自己的常量 —— 基座**无从核对**两个无组插件的别名会不会撞。核对不了的事只能整类互斥（粗一档拒绝并说出原因，细一档是静默的错误）。★ 这一档从前还有"端口由插件的 `preferredPort` 自己挑"这半个理由，**它已经不成立了**（那个钩子收掉了，无组会话的端口现在是基座的常量）—— 拆细的前提是插件把别名也交给基座声明 |

★ 因此**两个活跃会话必然占用两个不同的布局组**，而分区与端口各自分开 ——
这正是阶段 1 那句"实例段是常量时两份实例必然落在同一个目录里，各写各的"在
**运行期**的对应物。

★★ **今天够不到的一个推论**（写在 `slotOf` 的注释里，免得下一个人当成死代码）：
"同一个插件开两份"需要**两个布局组**，而今天没有第二个组的来源
（`app:setConnectionLayout` 的注释写明：单独建出来的组引用计数是 0，紧接着就被
`pruneLayouts` 回收，所以**刻意不提供**独立的建组通道）。所以这一版里同一个插件的
第二份必然撞在同一个槽上。见 `docs/KNOWN-ISSUES.md` 的 S24。
...
后来的版本又加上 `perInstance`），于是"删掉没人用的那一份"会把**正在用的**连根删掉。

★ 因为第二个落点**不由框架建**，"该有哪些身份"的入口必须按**注册表里的所有插件**算，
而不是照抄分区那一侧的"有界面的那些" —— 否则一个没有界面却会写数据的插件（sshd）
那份正被用着的数据一诞生就是孤儿，而界面上会给它一个删除按钮。

两个缺省都落在安全侧：不声明 `inherit` ⇒ 身份里带版本号（每个版本各一份，新版本读不到
旧数据）；不声明 `perInstance` ⇒ 身份里没有实例段（所有实例一份，于是同一份数据不会被
两份实例同时写）。**"同一个插件不许同时开两份"那条规则是后一个缺省的推论，不是另焊
上去的限制。**
