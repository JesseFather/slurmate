...
`client/src/main/backend-ssh.js` 是客户端与登录节点之间唯一的接缝，而它现在有
**两条**链路。下面每一条都对应一个**静默**的故障。

### `rpc()` 按"此刻有没有一条活的常驻通道"选路

```
有常驻通道 → 走它（ResidentChannel：一条 duplex 上多请求 + 收通知）
否则       → 走 exec（`slurmate rpc`，一个请求一条 channel）
```

**没有第三条路，也没有任何一处 try/catch 在"降级"。** 常驻通道没建起来、断了、
正在重开，都只是同一个函数的下一个分支 —— 于是"退回 exec"不需要被记得，
它就是代码的形状。

★★ **而"退回 exec"是必须支持的常态，不是容错**：登录节点上的 CLI 可能是旧的
（没有 `stream` 这个子命令，argparse 一行 usage、退出码 2）、ForceCommand 可能
拦了、守护进程可能没在运行。判据在 `session.js` 的推送看门狗 ——
**一条推送都没收到时，那边的对账 `STATUS_MS` 逐字回到从前。**

### 建起来 ≠ 能用：探针

`exec` 请求成功只说明**命令被启动了**。所以建起通道之后先问一句 `ping`，答得上来
才算它可用。★ 判据是"**对面**答了一个信封"，不是 `ok:true`（限流也是一次正常的
往返），而 `kind:'transport'` 必须排除掉 —— 那是**客户端自己**合成的信封，
通道一死 `request()` 就立刻拿它结账；不排除的话，一条刚建立就死掉的通道会被判成
可用，此后每一条请求都发进一条死通道，而 `resident` 还报 `true`。

★ `resident` 是**三态**：`null` 还没连上登录节点 / `false` 连了但没有通道 /
`true` 通道在。合成两态的话，启动阶段的日志会说出一句当时并不成立的话。

### `rid` **只从常驻通道上发**

它在守护进程那边是"这条连接从此收推送"的判据（见〈六之二〉），而 exec 那条路是
**一次性的**：发了 `rid`，守护进程会把推送写进一条马上要被关掉的连接，而推送与
响应**共用同一个输出队列** —— 一次恰好落在 tick 上的推送会被读成那条请求的应答。

所以 exec 那条路在发出去之前**复制一份并删掉 `rid`**（不改调用方的对象）。
写成"要求调用方别传"是不行的：不变量不能住在调用方的自觉里。

### 断开时，在途请求**全部**拿到答复

`ResidentChannel._die()` 给每一条在途请求一个 `kind:'transport'` 的信封。
★ 漏掉它的症状不是报错，是 `await` **永远不回** —— 而调用方（心跳那一类定时器）
会一轮轮往上叠新的请求，每 45 秒一条，永远不清理。

重开是**有上限**的（`STREAM_ATTEMPT_MAX`）：连着失败够多次就不再试。上限挡的是
"CLI 是旧的"那种**永久**失败 —— 它和暂时的失败（守护进程正在重启）现场一模一样，
区分不了，只能靠"试几次就不再试"。而这句话的反面同样重要：**每一次
注定失败的重开都是一次 exec**，也就是每分钟白 fork 一个 python。

★ 我们自己关的（探针失败、后端在拆）**不重开** —— 这个区分由 `_die(why,
deliberate)` 带出去；混起来的话，探针一失败就变成重开风暴。

### 推送：客户端只做两件事

```
1. 刷新视图（推送那份是"上一次对账时的视图"）
2. 在**实质变化**时去要一份权威视图（state / tunnel_target 变了）
```

别的判定（终态、releasing、隧道重建、心跳交叉校验）一律留在 `_statusOnce()` 里
—— 在推送那条路上重写一遍就是同一个状态机的第二份实现，而两份会漂。

★ `job_state` 变了**不**升级：推送那份来自本 tick 已经取过的作业信息（至多旧
2 秒），比现查一次还新。为一个再常见不过的状态迁移去 fork 一个 `scontrol`，
是把常驻通道省下来的东西又还回去。
...
| 规矩 | 读错的后果 |
|---|---|
| 推送里**缺**口令/作业内主机公钥 ⇒ **保留旧值**（推送结构上不带它们，是 `with_secret=False`） | 自动重登失败 ⇒ **页面打不开**，而界面上一切正常 |
| 推送里**找不到本会话** ⇒ 什么都不做（快照截断到最近 50 条） | 客户端自己拆掉隧道，界面上写"会话已不存在"，**而作业还在跑** |
| 换了 `session_id` ⇒ **不**保留旧口令 | 拿上一个作业的口令去打新作业 |

### 假后端照**同一条规则**推

变化的判据是**渲染结果本身**（一份指纹），不是"谁改过状态"；没变也每 30 秒推一条。
两个理由：前者让"忘了在某个改动点标脏"从构造上不存在（那个漏法的症状是**某个状态
在界面上永远不更新**）；后者是让客户端那条看门狗能把"安静"与"通道坏了"分开。

★ 这不是"多测几个函数"：常驻通道在真集群上可能起不来，**开发者模式是唯一能反复
走那条路的地方**。假站点不推的话，"推送那条路"就只有在真集群上才跑过。

## 六之四、多客户端：一个席位，和一次顶替

一个用户在同一个站点上可以有很多条 SSH 连接（终端、`slurmate wait`、VS Code 的
Remote-SSH、你的 `scp`），而**它们一条都不该被顶掉**。能被识别的只有**客户端实例**
—— 也就是"在某一台电脑上跑着的那个 Slurmate"。

### 身份是"这台电脑"的，不是"这次连接"的

`client.id` 落在客户端的数据目录里（`client-id.json`，与钉子、开发者模式设置同一类
独立小文件），一个后端会被重连很多次，而身份在它构造时就交给它了。
`name`（缺省是主机名）**只用于显示** —— 判等只看 `id`：一台机器改个名不该被当成
换了一台电脑，那会让它自己那份席位失效，症状是**它被自己顶掉**。

★ 开发者模式用的是沙盒目录，于是沙盒里的身份**与真身不同**。这是有意的：
开着开发者模式调试不该把你正在用的那个客户端顶掉。

### 认领的门槛与订阅的门槛是**同一条**

身份只在**带 `rid` 的请求**上被认领。理由与"订阅挂在 `rid` 上"逐字相同 ——
`slurmate rpc` 是常驻通道起不来时的**退路**，它永远不带 `rid`。它若也能认领身份，
客户端每退路一次就以同一个 `id` 报一次名，服务端会把**正在用的那条常驻通道**当成
"同一个客户端又连上来了"顶掉，于是每降级一次 exec 就把自己那条通道打掉一次
—— 症状是"通道时好时坏"，而两边各自的日志都自洽。

⇒ 认不出身份的连接**既不占席位、也永远不会被顶掉**。

### 席位按 `client.id` **首次出现**排

超编时从旧到新顶掉。**不是**按连接建立的先后，**不是**按活动度。

- 按活动度：两台电脑都在用时互相顶（你在这台上动一下鼠标那台就断），不可用。
- 按连接先后：一次**重连**（笔记本休眠醒来、网络闪断）会把没出任何问题的那台顶掉，
  而对方连自己为什么掉了都不知道。

★ 被顶掉的 `id` **同时从席位表里删掉**，于是它下次连进来算新人。这不是疏漏 ——
它是"被顶掉的那个禁止自动重连"必须成立的理由：少了那一条，两个客户端会互相顶，
**没有终点**。
...
新客户端靠已有的 `list` + `reattach` **接手心跳** —— 不需要为这件事新写一条路。

★ 而这条保证有**另一半在客户端**，且只能在客户端：被顶掉的客户端**绝不能**给这些
会话发 `goodbye`（`goodbye` 会让 `phase_release` 删 ACL **并 scancel 作业**）。
服务端分辨不了那个 `goodbye` 是谁发的（它按 `session_id` 记账，不按连接）。
所以 `SessionController.suspend()` 与 `stop()` 顶上那道闸是**承重**的，不是防御性的。

★ 还有两处容易漏的：被顶掉的记录**仍然占着本机的槽**（`occupied()` 看的是 controller
...
★★ **而这两处读的都是 `controller.suspended`。** 这个 getter 少写一次，两处就
**一起静默失效**（`undefined` 恒为假，一个字都不会报错）—— 这不是假设：这一版写
第一遍时就是这样，`session.js` 里只有 `_suspended` 与快照里那个同名的键，而
`index.js` 读的是第三个名字。**它是在收尾复核时才被看出来的。**
所以集群侧的用例里有一条**跨文件校验**盯着这一对名字（`get suspended()` ↔
`c.suspended`），因为这种漏法的形状就是"两个文件各写一半，中间没有东西保证一致"。

### 顶替通知**不看水位**

它是一次性的、不可重放的。被丢掉之后客户端只会看到一次**没有原因的**断线 ——
然后自动重连，而那正是拉锯的燃料。它很小，而水位之上压着的是**可以被丢掉**的全量
快照（见〈背压〉）。

### ★★ 被顶掉的客户端**一律拒绝发请求**，绝不退回 `exec`

这是这一节里最容易做错、后果最隐蔽的一格。退回 `exec` 的话，那个客户端会安静地
继续干活（心跳照发、状态照查、界面完全正常），而"你已经被另一台电脑接管了"
**一个字都不会出现** —— 一个只在协议层成立、在界面上看不见的状态。

★ 拒绝也是**对的语义**：手动点「停止会话」这类动作会让服务端 scancel 用户的作业，
而用户此刻以为自己在另一台电脑上操作。
...
`app:debug displaced`（调试面板上那个按钮）。真机上要两台电脑才能看到，
而它的**界面表现**恰恰是这个功能唯一要传达的东西。

## 六之五、集群信息：三层钟，一次查询服务所有连接

一屏只读的集群现状（`op_cluster`）：控制器健康、Slurm 版本、分区表、节点忙闲、
队列深度、GRES 清单，外加**自己那一份**（账户、可提交分区、公平份额、排队名次）。

### ★★★ 要害不是"多了几格"，是那几格**从哪来**

```
从前：请求 ⇒ fork `scontrol`
      每个 `partitions` / `submit` / `whoami` 各自一次，分区表那条连缓存都没有
      N 个用户 × M 条连接 ⇒ N×M 次，而答案本该是**同一份**

现在：tick ⇒ 刷新缓存（每层每周期一次）
      请求 ⇒ 只读缓存（一次 fork 都不发）
```

查询的对象（集群）本来就是共享的 —— 所以"最全面"与"省资源"在这里能同时成立。

### 三层钟

| 层 | 数据 | 周期 | 为什么在这一层 |
|---|---|---|---|
| 快 | 控制器健康 | 2 秒（= tick 周期） | 变得快、后果重；不加额外的钟 |
| 中 | 节点忙闲、队列深度 | 30 秒 | 回答"我要等多久"，不需要秒级 |
| 慢 | 分区表、GRES、账户、公平份额 | 300 秒 | 几乎不变，而它们对应的 fork 最贵 |

★ **分开的钟，不是一个 tick 干所有事。** 一个 tick 干所有事等于每种数据都被最急的
那种拖着跑：分区表 5 分钟刷一次就够，而控制器健康 2 秒就要。

★ **三档的缺省方向是"旧一点没关系，别把集群问爆"**：慢的那两层错了只是界面上
的数旧了几分钟。

### ★★ 钟的推进写在查询**之前**

`_due[key] = now + period`，而不是 `+= period`。写成 `+=` 的话，一次跑得比周期还久的
查询会让新的截止时刻仍然落在**过去** ⇒ 每一轮都重跑 ⇒ 一个慢集群把守护进程变成
一个不停 fork 的循环。这与 `run()` 里 `next_tick_at` 那条是同一个坑。

### ★ 查询失败按 `INFO_RETRY_SECONDS` 重试

不是等满自己那一层。慢钟上的东西失败一次就 5 分钟不再看，而"分区表读不到"是要
立刻自愈的瞬时故障（slurmctld 重启、munge 抖动）。代价是**只在坏着的时候**
每分钟多 6 次 fork。

★ 失败的结果**照样存进缓存**：三态规矩里"取不到"与"确实没有"是两回事，
调用方要看得出区别。

### ★★ 钟只能有一处

`Slurm` 从一个"带着三个各自 TTL 缓存（都是 300 秒）的查询层"收缩成**纯粹无状态的
查询层**：`_acct_cache` / `_part_cache` / `_gres_cache` 全部删掉。
理由是同一条规矩四份实现会漂 —— 而漂的形态是"分区表 5 分钟、账户 5 分钟、
GRES 5 分钟，谁也不知道为什么是这个数"。

★ 顺带解决的是另一件事：**TTL 缓存是请求驱动的**。到期之后**下一个请求**付那次
fork，而 50 个用户同时提交就是 50 次同时到期的 fork。钟跑在 tick 里之后，
fork 落在 tick 上，请求只读缓存。

### ★★ `Cluster` 拿的是"一个返回 `Slurm` 的可调用"，不是 `Slurm`

一个在构造时抓住 `Slurm` 的缓存层，会在那个对象被换掉之后继续问**旧的** ——
症状是"换了后端之后集群信息还是老样子"，一个字都不报，因为两边各自都工作正常。
`Sessiond` 传 `lambda: self.slurm`。

### ★★ 按用户的那一份：tick 预热 + 淘汰

`account_for` / `allowed_partitions` / `fairshare` 是**按 uid** 的（共享的那几格
对所有 uid 逐字相同）。tick 替**现在连着的**用户预热它们 —— 判据是"连着"而不是
"有会话"，因为一个刚连上、还没提交任何东西的用户打开面板时同样要立刻有东西看。

★ `prune()` 丢掉**很久没人问过**的按用户条目。判据是"最后一次有人碰过它"，
而那条记录写在 `_pull()` 的**最前面**（在到期判断之前），并且 `_pull` 对共享的键
也记 —— "共享的永远不淘汰"这条规则只在 `prune()` 里写一遍。
★ 实测踩过两次：① 只在请求侧的 `_read` 里记，于是"刚被 tick 预热过"的条目看起来
像没人问过，每个周期被丢一次再查一遍；② 在 `_pull` 里再拦一道"只记按用户的"，
那是同一个判据的第二份实现 —— 去掉它**一条用例都不红**，也就是说那一道守卫
没有任何东西盯着。

### ★★ 节点忙闲：flag 只做展示

`sinfo -N -h -o "%P|%N|%t"` 聚合成"每分区每个 base state 几台"，后缀
（`*` `~` `#` `!` `%` `$` `@` `^` `-` `+`）**原样带出去、单独一列**。

★ 它们**跨 Slurm 版本含义不一致**：同一份 manpage 上 `~` 是"presently in powered
off"，别的文档把它说成省电模式，还有版本把它关联到 `COMPLETING`。拿它们判定等于
把"我记得的那个版本"当成协议。
★ 剥法（从右往左剥）成立的前提是**后缀字符集与紧凑状态名不相交**，
`NODE_FLAG_CHARS` 旁边有一条用例逐字盯着这件事。
★★ **`-h` 不能省**：`sinfo` 与 `scontrol show … -o` 不一样，它会打一行表头，
而那一行会被当成一个名叫 `PARTITION` 的**假分区**。（真集群实测抓到的 —— 手写的
测试桩本来没有表头，所以它一直是绿的。）

### ★★ 队列：「其余那一档」不并进 running

一次 `squeue -h -o "%i|%P|%t|%u"` 回答所有人：每分区的排队/在跑计数、排队顺序、
按用户分的列表。

- 数组作业算**一条**（Slurm 自己也是把它当一个作业排的）。
- `CG` / `S` / `ST` 那些**单独一档**，不并进 running —— 并了之后"这个分区忙不忙"
  就开始说谎，而那是这一格唯一的用途。
- "我排第几"从共享的两份列表**现推**，不额外 fork。报的是**第几位**，不是
  "还要等多久"（后者取决于分区此刻有多少空闲节点，报出来像承诺）。

### ★ `history` 按需拉，不进任何一层钟

`sacct` 是这一组里最贵的一条，而它回答的"过去发生了什么"**不会自己变新** ——
没有调用者就没有这个 fork。时间窗由 `-S now-Ndays` 下推给 Slurm（★ 必须是
`days`，`now-7d` 是非法的），条数由我们自己截。
