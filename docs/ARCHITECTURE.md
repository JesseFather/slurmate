...
| 半边 | 在哪 |
|---|---|
| 作业侧 | `run.sbatch` 里的一个 `start_*` 函数，由 `start_service` 用 `case` 分派 |
| 服务端 | `slurmate-sessiond` 的 `PluginSpec` 表（id、版本、短名、默认资源…） |
| 客户端 | `client/src/main/plugins/<短名>/`（清单 + `client/index.js`） |
| 站点策略 | `slurmate.conf` 里的 `[plugin:<短名>]` 块（开不开、默认资源、可执行文件） |

现在有两个：`code-server`（浏览器里的 IDE）与 `sshd`（用户态 ssh，给原生 VS Code
Remote-SSH / codex 这类**要求 ssh 连接**的工具用）。见
...
插件只回答一个问题：**这个会话该怎么用。** code-server 建视图并自动登录；
sshd 把本地 ssh 配好。它们拿到的是框架显式递过去的一组能力（客户端那边叫 `ctx`），
而不是整个模块作用域。
...
4. **客户端自己的插件目录里有一个坏插件**（清单不是 JSON、`id` 不是 ULID、有认不得
   ...
   停得掉）。这条防**意外**，不防**恶意**：防恶意要靠第 3 层那四条护栏。
...
四张表，各在一侧，**框架一行都不用改**（`index.js` / `session.js` / `windows.js` /
守护进程的生命周期逻辑都不认识任何插件名）：

| # | 在哪 | 加什么 |
|---|---|---|
| 1 | `cluster/run.sbatch` | 一个 `start_<短名>` 函数，以及 `start_service` 的 `case` 里一个分支 |
| 2 | `cluster/slurmate-sessiond` 的 `BUILTIN_PLUGINS` | 一条 `PluginSpec`（**铸一个新的 ULID** 当 id、短名、默认资源、要不要公钥…） |
| 3 | `client/src/main/plugins/<短名>/` | `plugin.json`（id 与 #2 **逐字一致**）+ `client/index.js` |
| 4 | `/etc/slurmate/slurmate.conf` | 一个 `[plugin:<短名>]` 块。**这是站点的决定**，不进仓库 |

★ #2 与 #3 的 id 是同一对常量写在两种语言里（它们必须能各自独立启动，所以无法只写
一处）。这靠**一条用例**钉住 —— `cluster/test-sessiond-logic.py` 会读客户端的
`plugin.json` 逐字比对 id / 版本 / 短名。漂了的后果不是崩溃，而是**静默接错**：
会话记的 `<id>@<版本>` 在客户端池里查不到，于是界面只解释、不动作，用户看到的
是"作业起来了但界面一片白"。
...
| 层 | 是什么 | 客户端执行服务端代码 | 状态 |
|---|---|---|---|
| 1 内建 | 代码随本项目发布（现在这两个） | 否 | **本版** |
| 2 声明式 | 站点用一个 manifest 描述"起什么命令、暴露哪个端口、客户端怎么接" | 否 | 下一阶段 |
| 3 代码 | 站点分发 `client/index.js` | **是** —— 需要签名、钉公钥、逐插件同意、进程隔离 | 下一阶段 |

**本版只做了这一层的结构**：内建的两个已经搬进「一个目录 + 一份清单」的形状
（`plugins/<短名>/plugin.json` + `client/index.js`），与站点分发的插件走**同一条
加载路径** —— 后者只是多看一个目录。客户端也已经会扫 `~/.slurmate/plugins/`，
但**没有任何东西会往那里装**（还没有取插件文件的 op）。所以第 2、3 层是"多一个
扫描根、多一个签名验证"，不是两套机制。

第 3 层是唯一需要付出信任代价的一层：它会多出一条**反向**的信任路径（集群管理员 →
用户的工作站）。今天那条路径**不存在** —— 客户端发往登录节点的命令串是编译期常量，
服务端收到的永远只是数据。所以那一层要做就必须带齐四条护栏，否则它就是一个安静的
后门。
