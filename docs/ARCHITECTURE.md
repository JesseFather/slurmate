...
| 3 代码分发 | 同上，但插件带 `client/index.js` | **是** —— 需要签名、钉公钥、逐插件同意、进程隔离 | **v0.6：前三条做了（钉公钥还差接线），第四条欠着**（见下） |
...
| **钉公钥** | ✅ 判据与表都落地了：`~/.slurmate/pinned-keys.json`（键是 **`id`**）+ `keyVerdict`。🔁 **还差接线** —— 它们还没有调用方，接上的是"客户端按包安装"那一段。★ 它挡"事后偷换"与"第一次给的就是坏的"两者；一致性台账（`cfg.trustedPlugins`）仍管前者，两件事不写在同一个句子里 |
| **签名** | ✅ 做了：`packer` 的 `keygen` / `sign`、跟着源码树走的**血统表**（§2.5）、以及上面那张按 id 的钉表。🔁 **站点侧的验签**接在安装器那一段（见 S6）。★ 原先"不做站得住"的条件是三个前提，而**其中两条已经不成立** —— 论证见 [SECURITY.md](../SECURITY.md) |
