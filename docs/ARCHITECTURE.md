...
| 客户端 | `<插件目录>/client/index.js` | 客户端的注册表 —— **可以没有**（那就是声明式插件） | 客户端启动时扫池；站点分发的那一份由对账取回来，**过了同意闸才加载** |
| 作业侧 | `<插件目录>/job/start.sh` | **没有任何运行时读者** —— **可以没有**（那这个插件就提交不了） | deploy.sh 部署时**逐插件织一份**作业脚本 |
| 站点策略 | `slurmate.conf` 里的 `[plugin:<短名>]` 块（开不开、默认资源、可执行文件） | 守护进程 | 守护进程启动 |

★ **基座不带任何插件，两端都是。** 客户端那边 `client/src/main/plugins/` 里只有框架
（注册表、铸造 id 的 `ulid.js`、安装器），插件落在两个**根**里 ——
`~/.slurmate/site-plugins/`（站点分发的，v0.6 起是默认那条路，见〈插件从哪来〉）
与 `~/.slurmate/plugins/`（本机自己的，**默认不加载**，走 `plugins/install.js`）；
...
★ 于是「加一个插件」只是**放一个目录 + 跑一次 `deploy.sh --plugins-src`**。
...
答得了。所以客户端那边是一个**池**（`~/.slurmate/plugins/`）：任意站点的安装都丢
进去，任意站点的使用都从里面取，卸载就是把它拿掉。
...
`~/.slurmate/plugins/<id>/<版本>/`，相对路径指不到客户端；就算指得到，那种依赖也
...
| 2 | `sudo bash cluster/deploy.sh --plugins-src <那个目录的父目录>` | 装进站点 + 为它织一份 `<prefix>/share/slurmate/jobs/<ULID>.sbatch` |
...
那个目录是取回来的，还是用户自己放进去的，对加载器是同一件事（两个**根**，
两条来路，见〈插件从哪来〉）。
...
| 层 | 是什么 | 客户端执行服务端代码 | 状态 |
|---|---|---|---|
| 1 本地安装 | 用户/管理员把一个插件目录交给客户端，它落进池 | 否（除非那个插件带 `client/index.js`） | 有，但**默认关着**（开发者模式） |
| 2 站点分发 | 站点把插件的文件推给客户端 | 同上 | **v0.6** |
| 3 代码分发 | 同上，但插件带 `client/index.js` | **是** —— 需要签名、钉公钥、逐插件同意、进程隔离 | **v0.6，四条护栏做了一条半**（见下） |
...
★ **上面这一行"读包"落的是能力，不是路径**：这一版**不动线、不动布局** ——
站点仍然报 `files`、客户端仍然按 `files` 取。把安装路径切到包上是后面的事，
而顺序是刻意的：**读包的能力必须先于任何一条线落地**，否则中间会开一个窗口，
那个窗口里客户端拿到的是它读不懂的字节，而它唯一能说的话是"校验不过"。
...
| **钉公钥** | 🔁 **用等价物替代**：一份 TOFU 一致性台账（`cfg.trustedPlugins`），与 `hostKeys` 同一先例。它挡得住"事后偷换"，**挡不住"第一次给的就是坏的"** —— 摘要是**一致性**判据，公钥是**认证**判据，两件事不要写在同一个句子里 |
| **签名** | ❌ 没做，**并给出了不做站得住的条件**：单站点 + 站点即作者 + 站点那份由 root 从源码装，三条**同时**成立时签名不增加安全性。**任一条不成立，签名就从"锦上添花"变成"唯一能区分作者的东西"** —— 那条判据写在 SECURITY.md 里，不是含糊过去的 |
| **进程隔离** | ❌ **还欠着**。`client/index.js` 仍然 `require()` 进 Electron 主进程，完整 Node 权限、没有沙箱 |

★ **四条里没做的只有两条（签名、进程隔离），而其中只有一条需要一次重构**
（进程隔离）。在那之前，**同意界面的措辞就是唯一的安全边界** —— 所以它把话说满了：
没有进程隔离、那段代码会在你这台机器上跑、以及第二次之后旧摘要 → 新摘要的对照。

★ **上面这张表描述的是 v0.6 发布时的状态，它至今仍然准确。** 但"签名"那一行
**已经被后来的决定推翻了**：[PLUGIN-SPEC.md](PLUGIN-SPEC.md) 的 **§5.4** 把签名
（以"钉公钥"的形式）写成客户端**必须履行**的一条保证，因为发现了三个前提射程之外
的第四个场景 —— **同一身份下面"作者换了人"**。那件事只有签名判得了（摘要是一致性
判据，分身升个版本号在台账上就是一条正常条目）。**那一行不要照着往下写代码。**

**一格一格地更正**：签名的**字节格式**已经定死了（同一份规范的**附录 A**），
**验签**已经在两处实现了（客户端、打包器）—— 但**按 id 的钉表**没做、`packer`
的 `sign` 动词没做、而**站点的读方刻意不验签**。三者各自的理由与后果记在
[KNOWN-ISSUES.md](KNOWN-ISSUES.md) 的 **S6**。

**第 2 层与第 1 层不是同一条路。** 这是本版改掉的一处说法：`plugins/install.js`
的 `installFrom(dir)` 的语义是"**用户**挑的目录装进**用户自己的**池"，而分发是
"**站点**说了算、按引用计数回收、换入前要过同意闸" —— 两者的语义相反（前者"绝不
覆盖内容不同的同版本"，后者要的正是"站点换了内容就得重新同意"）。所以分发**不经过**
`installFrom`，它落在**另一个根**上：
...
★ 两个根不是洁癖：回收只该删**站点拥有的**那些。用户手装的那一份不在任何站点记录
里、引用数天然是 0 —— 合并成一个目录就等于"回收会把用户自己的东西删掉"。所以
**"池的模式"指的是布局与身份模型**（一个目录、`<id>/<版本>/`、按 `id` 认身份），
**不是"只有一个目录"**。
