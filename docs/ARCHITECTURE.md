...
| 层 | 是什么 | 客户端执行服务端代码 | 状态 |
|---|---|---|---|
| 1 本地安装 | 用户/管理员把一个**包**交给客户端，它落进池 | 否（除非那个插件带 `client/index.js`） | 有，但**默认关着**（开发者模式） |
...
只是"把一批包一起装"），守护进程**两条投递方式都通**（`files` 逐份取、`package`
整包一次取）。顺序是刻意的 —— **读包的能力先于任何一条线落地**，否则中间会开一个
窗口，那个窗口里客户端拿到的是它读不懂的字节，而它唯一能说的话是"校验不过"。

★ **客户端那一半也切过来了**：一次对账优先走 `package`（1 + 1 + 1 次 RPC），
池里每个版本是**两样挨着** —— `<id>/<版本>/`（解出来的树，`require()` 用的是它）
与 `<id>/<版本>.splug`（包本身）。`files` 那条路留着当退路，但**只在"这个包用的是
本客户端读不懂的格式"时**才会走。★ 留着的理由不是客气：它是已部署的 v0.6 站点与
老客户端共用的那条，删掉它是一次不兼容变更，所以排在最后一段（见 PROTOCOL.md 的
〈七、兼容性〉与 [KNOWN-ISSUES.md](KNOWN-ISSUES.md) 的 **U7** —— 真集群上的一次
完整分发还没跑过，在那之前旧路一直能当退路）。
...
| **钉公钥** | ✅ **接上了**：`~/.slurmate/pinned-keys.json`（键是 **`id`**）+ `keyVerdict`，在用户点同意的那一刻钉（`pinPluginKey`），此后每一份都过判据。★ 逐份那条路**也判** —— 它发的是散装字节、证不了签名者，所以钉过的 id 在那里一律拒；不判的话"让整包失败"就是一个绕开钉子的开关。★ 它挡"事后偷换"与"第一次给的就是坏的"两者；一致性台账（`cfg.trustedPlugins`）仍管前者，两件事不写在同一个句子里 |
