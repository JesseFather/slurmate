...
| 3 | `sudo bash cluster/deploy.sh --plugins-src <那个目录的父目录>` | 装进站点 + 为它织一份 `<prefix>/share/slurmate/jobs/<ULID>.sbatch` |
...
成品里当然有那个插件的代码 —— 但**只有那一个**，而哪一份对应哪个插件由守护进程按
清单里的 `id` 决定（第 17 节逐插件断言 argv 末项就是它自己那一份）。
...
| 层 | 是什么 | 客户端执行服务端代码 | 状态 |
|---|---|---|---|
| 1 本地安装 | 用户/管理员把一个插件目录交给客户端，它落进池 | 否（除非那个插件带 `client/index.js`） | **本版** |
| 2 站点分发 | 站点把插件的文件推给客户端 | 同上 | 下一阶段 |
| 3 代码分发 | 同上，但插件带 `client/index.js` | **是** —— 需要签名、钉公钥、逐插件同意、进程隔离 | 下一阶段 |

**本版做的是第 1 层，而且它是第 2 层的**同一条路**：`plugins/install.js` 的
`installFrom(dir)` 不关心那个目录是从哪来的 —— 站点分发只是把文件先落到本地一个
临时目录再调同一个函数。所以第 2 层是"多一个取文件的方式 + 一条签名验证"，不是
一套新机制。

今天**没有任何东西会从集群取插件文件**（没有那个 op）。客户端会扫
`~/.slurmate/plugins/`，而里面有什么完全取决于用户装了什么。
