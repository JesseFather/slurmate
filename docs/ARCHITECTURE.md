...
>
> ⚠️ **文中的 `文件:行号` 引用会随代码漂移。** v0.3 的插件重构让
> `cluster/slurmate-sessiond` 长了 500 多行，此前写下的行号因此整体偏移。
...
| 服务端 | `slurmate-sessiond` 的 `PluginSpec` 表（名字、默认资源、要不要公钥…） |
| 客户端 | `client/src/main/plugins/<名字>.js` |
| 站点策略 | `slurmate.conf` 里的 `[plugin:<名字>]` 块（开不开、默认资源、可执行文件） |
...
4. **客户端自己的插件目录里有一个坏文件**（语法错、缺字段、名字与文件名不一致）
   → 跳过它、记一条、其余插件照常工作。
...
| 2 声明式 | 站点用一个 manifest 描述"起什么命令、暴露哪个端口、客户端怎么接" | 否 | 后续版本 |
| 3 代码 | 站点分发 `client/index.js` | **是** —— 需要签名、钉公钥、逐插件同意、进程隔离 | 后续版本 |
