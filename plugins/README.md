...
| `perInstance` | **只有一份** —— 所有实例共用，于是天然不许同时开两份 | 每份实例一份（**实例 = 布局组**，所以它要求同时有 `layout: true`）。★ 写了它，你那份数据就**跟着那个组的引用计数走**：最后一个用那个组的连接被删掉（或被切到别的组）时，它会被一起删掉 —— 见下面〈两个落点〉 |
...
数据（即声明了 `perInstance` 的），**在那个组被回收时自动删掉** —— 最后一条指着那个
组的连接被删掉、或被切到别的组的时候。★ 所以声明 `perInstance` 等于说"这份数据跟着
那个组的引用计数走"；不声明（sshd 那一档）则相反 —— 它只有一份、不属于任何组，
