...
那一节里才会出现「从一个包安装…」「打开插件目录」「重新扫描」。

★ **那个入口选的是一个 `.splug` 文件，不是一个目录**（§5.1：插件进池子只有
"安装一个包"这一个动作）。所以本机开发循环里多一步：

```
packer init plugins/<你的插件>     # 第一次：铸 id、写回清单（然后提交它）
packer build plugins/<你的插件>    # 每次改完：从 **commit** 打一个包出来
```

然后界面上「从一个包安装…」选那个 `.splug`。★ 手打的那个包**不进 git**
（二进制包进 git 等于代码评审死掉）—— 仓库里留的是源码。
