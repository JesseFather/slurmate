...
### `contributes.concurrent` —— 能不能同时开两份（**必填**）

| 值 | 意思 |
|---|---|
| `true` | 允许同一时刻有两个这个插件的会话。第二个拿一份**临时的**副本：新的本地端口、空的浏览器存储（新 origin）、你那份数据的**快照**，而它的一切在这个会话结束时消失。★ 它**要求 `layout: true`** —— 实例键就是布局组。 |
| `false` | 同一时刻只能有一个。不要布局组的插件一律写这个（它没有第二份实例键的来源）。 |

★ **不写 ⇒ 安装会被拒绝。** 这一格**刻意没有缺省**：缺省无论取哪一边都是基座替你
表态 —— 取"不能"，一个真能多开的作者永远不知道为什么只能开一份；取"能"，一个没想过
的作者会得到一个静默的第二份。

★ **「不要布局组」与「不能同时开两份」是两件事**，别混：`layout` 说的是"这次会话要不要
一个本地端口与一份浏览器存储"，`concurrent` 说的是"你的插件能不能有两个同时活着"。

★ **多开时只有一份能攒数据**（"第一份"，开局那一刻先来的那个），其余的都是临时的。
"两份各自攒各自的东西"这一格今天给不了 —— 那就写 `false`。

★ **谁是"第一份"只在开局那一刻定**：你点「开始会话」时，如果那个布局组上已经有一条
这个插件的会话在跑，你新起的这一条就是临时的。★ 而**已经开着的那几份不会因为第一份
结束而"升格"** —— 它们是什么就是什么；下一个**新开的**会话才认领第一份。这一条是
故意的：一个正在跑的会话手里攥着它那个组（分区、数据目录、外面那个 origin 都从它
算），把身份挪到它头上等于在运行时搬家。

### `contributes.data` —— 你的运行时数据

**不写这一段是完全正常的。** 它只有一个键、而且有缺省，所以不声明只是"保守"，不是
"少配了一样东西"（仓库里两个内置插件只有一个写了它）。
...
（这一格基座答得了 —— "不共享"永远安全 —— 所以缺省可以填。而 `concurrent` 那一格
答不了，所以它必填。）

★ **实例段（第三段）存不存在，由 `concurrent` 决定**：写 `true` ⇒ 有这一段（两份实例
必须落在两个目录里，不然是**静默损坏**，两边都以为自己成功了）；写 `false` ⇒ 整段
不存在。
...
| 落点 | 谁写 | 怎么拿 |
|---|---|---|
| **浏览器存储**（页面里的 localStorage、cookie…） | 你的页面自己写 | 框架建好了，你什么都不用做 |
| **插件的数据目录** | **你**在 `client/index.js` 里写文件 | `ctx.dataDir()` 给你一个绝对路径 |

★ 两个落点用的是**同一个身份**，所以它们是"同一份数据的两半" —— 用户在界面上看到的
...
★★ **而且"没有人在用"有两种发生方式，其中一种你按不到按钮**：属于某个布局组的那份
...
★ **临时副本的那份数据是"快照"，不是"共享"**：客户端在你开局那一刻把第一份**整个
目录拷一份**（含子目录）当作起点，之后两份各写各的 —— 临时那份改什么都不会影响
第一份，反之亦然。★ 而客户端**在拷不出来的时候照样把会话起起来**（起点是空的 +
一条提示）：一份临时副本因为"读不到起点"而拒绝开局，代价比"起点是空的"大得多。

★ **这份数据在用户的机器上是"看得见、删得掉"的**：客户端会列出**没有任何连接在用**的
那些，并给用户一个删除按钮。有了上面那条自动回收之后，列出来的多半是**残留**
（属于一个已经不在本机的插件版本、或属于一个已删的插件）。

★ `inherit` 的组名是**你自己起的**，两个插件正好都叫 `editor` 完全正常 —— 身份的头一段
是插件 id，它俩不会撞上。

★ 磁盘上那个目录名是**折叠过**的（插件 id 那一段是小写），这是 Electron 的行为
（`MakePartitionName`）。你不需要管它，但如果你在排障时去 `Partitions/` 下找自己那份
数据，别按大写去找。
