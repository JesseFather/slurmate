...
#
#    2. **函数名一律带 `_code_server` 后缀**。一插件一份之后，插件**之间**不会
#       互相覆盖了，但盖掉**宿主自己**的函数仍然是静默的（症状是"清理没跑"
#       "日志少了几行"这类指不回任何一个文件的现象）。宿主自己的函数都是没有
#       后缀的动词（log / cleanup / write_session / pick_port_and_start …），
#       两套命名不会撞。
#
