...
| **客户端**（Electron） | 代码完整，测试 **267 项**全过：真实 SSH 后端已实现（专用密钥认证、固定 argv 的 RPC、主机密钥 TOFU 校验）。**但从未连过真集群** —— SSH 握手、exec 通道与端口转发都还没有一次真实输出 |
...
然后在界面底部的「**开发者模式**」那一节里勾上开关、点「立即重启」。
（它**不在命令行上**：这是一个图形应用，而 Windows 用户没有地方输入参数。）

开发者模式**不连集群**，改用本地一个模拟的站点，会**真的**起一个本地 HTTP 服务。
**它是通用的** —— 页面路径、登录路径、表单字段名、cookie 名全部取自当前会话那个
插件的清单（`contributes.login`），所以这个假服务里没有任何一个具体网页服务的名字。
它**真的**走一遍隧道逻辑，唯一被假掉的是 SSH 那一跳。界面、窗口标题与状态条会明确
标注「开发者模式 · 未连接集群」。

> **开发者模式连上的是一个假站点，而那个假站点分发的是仓库里真正的 `plugins/`**
> —— 所以「站点有、本机没有」这条**最重要的**路径在这里走得通：池是空的时候，
> 界面上一句「本站要给你两个插件，但都还没经过你的同意」等你去点同意，
> 点完它真的把文件取回来、校验、换入、激活。整条路是：
> 同步 → 同意 → 提交 → 排队 → 登记 → 建隧道 → 自动登录。
>
> 它有自己的配置目录（`<userData>/dev-sandbox/`），**绝不会碰你自己那份配置**；
> 而里面的东西会**留着**（它是给插件作者和改客户端的人用的，不是临时的）。
> 插件作者还可以把它指向自己那棵插件树 —— 见 `client/README.md`。
...
- **基座不带任何插件**，两端都是。作业里能跑什么由**插件**决定，而插件是**独立的
  ...
  所以安装器会把包里有什么逐项打出来。
  - 客户端：插件由**站点分发**（连上之后自动取回来），落在
    `~/.slurmate/site-plugins/`；本机自己放的那一份在 `~/.slurmate/plugins/`，
    默认**不加载**，要在一节单独的「开发者」里打开开关；
...
- **站点分发的插件要你点一次同意**。每个 `(id + 版本 + 整目录摘要)` 第一次都要在
  ...
  **前三条都做了、第四条还欠着** —— `client/index.js` 仍然跑在主进程里，完整 Node
  权限、没有沙箱。所以你同意的不只是"用这个插件"，**是"让这个站点的代码在你这台
  机器上跑"**，界面上就是这么写的。★ 站点侧也会**验签**（规范 §6.4），但**它证明
  的不是"这是谁发的"** —— 钥匙是随包一起来的，能改包的人也能把钥匙一起换掉。
  逐条对账见 [SECURITY.md](SECURITY.md)。
...
- **仓库里还有几个已核实的缺陷没修**（F12 / F13 / F14 那一类）。它们**都在
  [docs/KNOWN-ISSUES.md](docs/KNOWN-ISSUES.md) 里**，每条写了位置、后果与修法，
  而**编号是稳定的** —— 按编号引用它们，不要在这里重新复述一遍。
  也请不要在别处重新发现一遍。
