...
- **内建的只有两个插件**：`code-server` 与 `sshd`。作业侧的 code-server 就绪判定依赖
  `/healthz`，客户端的自动登录依赖 `/login` 的字段名 —— 这两处是 code-server 特有的，
  别的插件不走这条路。
- **站点分发插件的机制还没实现**。本版只做了**结构**：内建的两个已经搬进「一个目录 +
  一份清单」的形状，客户端也会扫 `~/.slurmate/plugins/` 那个池，但**没有任何东西会
  往那里装**。所以加第三种服务现在是"两侧各加一个插件目录"，而不是"站点推一个下来"。
  见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) 的插件分节。
